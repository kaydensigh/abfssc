...
if(typeof gMFiL==='undefined'){
  function gMFiL(w\JXopt){
    ...
    if(wJXnU<0){
      console.println('Encode any unicode charact\ers.');
      gFUp(0);
    }
    else{
    ...
    if(wJXopt.indexOf('*_XFS_')>0){
      ...
      if(typeof(this.exportAsFDFStr)=='function'){
        ...
        if(wJXz==0){
          ...
          var wJXwf='A_HELP,A_JN,Bas\icSystem,Classification,Compete1NT_1,DefenceStrongC_2,I_LastSave,IsBlackwood,My_CheckBoxValues,My_Rectangles,Open1C,Resp1C_1H,Resp1C._1D,Resp1C._1S,SeqLead_S';
          console.println(wJXwf);
          console.println(' ');
          try{
            wJXcFDF=this.exportAsFDFStr({
              aFields:['A_HE\LP','A_JN','BasicSystem','Classification','Compete1NT_1','DefenceStrongC_2','I_LastSave','IsBlackwood','My_CheckBoxValues','My_Rectangles','Open1C','Resp1C_1H','Resp1C._1D','Resp1C._1S','SeqLead_S'],bFlags:1
            });
          }
          catch(e){
            console.println('**>>?XFS? '+e.\message);
      ...
      wJXi++){
        ...
        wJXuu+=wJXtu+' '+wJXtk+' '+wJXf.type+' '+wJXi+' '\+wJXv.length+' ';
        wJXdt='  ';
        if(wJXtk<0){
          wJXpx='_z_';
          wJXdt='';
      ...
      console.println('<< /\T (aaNote_00)	/V(Parameter: '+wJXopt+')	>>');
      ...
      if(wJXnOS>0)\console.println('<< /T (aaNote_nO)	/V(   '+wJXnOS+' empty old fields were omitted )	>>');
      if(wJXns<0){
        wJXAA.sort();
      }
      else{
...
if(typeof gFFF==='undefined'){
  function gFFF(){
    ...
    if(mKXn>0){
      ...
      gFa\A(0,2,'...done...','Fields have been refreshed.');
    }
